...
from typing import *
import numpy as np
from fairpyx.algorithms.Optimization_Matching import FaStGen, FaSt  # Ensure your FaStGen and FaSt are properly imported
from fairpyx import AllocationBuilder  # Assuming this is where AllocationBuilder comes from
...
def evaluate_algorithm_on_instance(algorithm, instance):
    if algorithm in [FaStGen, FaSt]:
        alloc = AllocationBuilder(instance=instance)
        if algorithm == FaStGen:
            items_valuations = instance._valuations  # Assuming you need to pass this
            allocation = algorithm(alloc, items_valuations)
        else:
