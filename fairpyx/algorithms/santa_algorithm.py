...
def bundle_value(agent: str, bundle: tuple, valuations_frozenset: frozenset) -> float:
    # valuations_frozenset is a collection of tuples of the form (agent, item, value)
    vals = {(a, i): v for (a, i, v) in valuations_frozenset}
    return sum(vals.get((agent, item), 0) for item in bundle)

@lru_cache(maxsize=None)
def cached_combinations(items_tuple: tuple, r: int) -> tuple[tuple]:
    """
...
def cached_thin_bundles(player: str,
                        thin_items_t: tuple,
                        threshold: float,
                        vals_f: frozenset) -> tuple:
    """
    Returns all minimal bundles of thin_items for the given player at a specified threshold.
    """
    # Construct a dictionary (dict) for the values
    ...
    for r in range(1, len(thin_items_t)+1):
        for b in combinations(thin_items_t, r):
            ...
            if total < threshold:
                continue
            # Minimality check
            if any(total - vals[(player,x)] >= threshold for x in b):
                continue
            res.append(frozenset(b))
    return tuple(res)
...
def solve_configuration_lp(valuations: Dict[str, Dict[str, float]], threshold: float) -> Dict[str, str]:
    ...
    # Convert the valuation map into a frozenset of (agent, item, value) tuples
    # for compatibility with the bundle_value function
    flat = frozenset(
        (agent, item, valuations[agent][item])
    ...
    bundles = {
        agent: [
            frozenset(bundle)
            for r in range(1, len(items_t) + 1)
            for bundle in cached_combinations(items_t, r)
            if bundle_value(agent, bundle, flat) >= threshold
        ]
        for agent in agents
    }

    # keep only minimal bundles
    for agent, agent_bundles in bundles.items():
        minimal = []
        for b in agent_bundles:
            if not any((b2 < b) for b2 in agent_bundles if b2 != b):
                minimal.append(b)
            bundles[agent] = minimal
...
def build_hypergraph(valuations: Dict[str, Dict[str, float]],
    ...
    # Prepare cache: tuples of thin_items and valuations
    items_t = tuple(sorted(thin_items))
    flat_vals = frozenset(
    (agent, item, valuations[agent][item])
    for agent in valuations
    for item in valuations[agent]
