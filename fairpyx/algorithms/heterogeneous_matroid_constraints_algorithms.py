...
def categorization_friendly_picking_sequence(alloc: AllocationBuilder, agent_order: list, item_categories: dict,
    ...
    remaining_category_agent_capacities = {agent: agent_category_capacities[agent][target_category] for agent in
                                           agent_category_capacities if agent_category_capacities[agent][target_category] != 0}  # should look {'Agenti':[k:int]}
...
def capped_round_robin(alloc: AllocationBuilder, item_categories: dict, agent_category_capacities: dict, initial_order: list):
    """
    ...
    categorization_friendly_picking_sequence(alloc, current_order, item_categories, agent_category_capacities,'c1')  # this is RR without wrapper
...
def two_categories_capped_round_robin(alloc: AllocationBuilder, item_categories: dict, agent_category_capacities: dict,
    ...
    """
        ...
        -> F-EF1 (feasible envy-freeness up to 1 good) allocation
            ...
            >>> valuations = {'Agent1':{'m1':10,'m2':3,'m3':3},'Agent2':{'m1':3,'m2':1,'m3':1},'Agent3':{'m1':10,'m2':10,'m3':10}} #TODO change v(agent1(m1))=10 in papers
            >>> divide(algorithm=two_categories_capped_round_robin,instance=Instance(valuations=valuations,items=items),item_categories=item_categories,agent_category_capacities= agent_category_capacities,initial_order=order)
            ...
            >>> valuations = {'Agent1':{'m1':1,'m2':2,'m3':3,'m4':4,'m5':5,'m6':6},'Agent2':{'m1':6,'m2':5,'m3':4,'m4':3,'m5':2,'m6':1},'Agent3':{'m1':5,'m2':3,'m3':1,'m4':2,'m5':4,'m6':6}} # TODO change values in papers
            >>> divide(algorithm=two_categories_capped_round_robin,instance=Instance(valuations=valuations,items=items),item_categories=item_categories,agent_category_capacities= agent_category_capacities,initial_order=order)
            {'Agent1':['m2','m3','m4'],'Agent2':['m5'],'Agent3':['m6']}
            >>> # m1 remains unallocated unfortunately :-(
            """
    current_order = initial_order
    categorization_friendly_picking_sequence(alloc, current_order, item_categories, agent_category_capacities,'c1')  #calling CRR on first category
    current_order.reverse()#reversing order
    categorization_friendly_picking_sequence(alloc, current_order, item_categories, agent_category_capacities,'c2')  # calling CRR on first category
if __name__ == "__main__":
    import doctest
    doctest.testmod()
