...
def second_improved_high_multiplicity_fair_allocation(alloc: AllocationBuilder):
    """
      ...
      >>> divide(second_improved_high_multiplicity_fair_allocation, instance=instance)
    ...
    while alloc_X is not None:
        ...
        logger.debug(f'The envy matrix of the allocation {second_improved_calculate_envy_matrix(alloc, alloc_X)} ')
        ...
        if alloc_Y is None:
            ...
            logger.debug(f'The envy matrix of the Pareto dominating allocation {second_improved_calculate_envy_matrix(alloc, alloc_Y)} ')
            logger.debug(f'The values of the first allocation {second_improved_calculate_values(alloc, alloc_X)} ')
            logger.debug(f'The values of the Pareto dominating allocation {second_improved_calculate_values(alloc, alloc_Y)}')

            constraints_ilp.extend(second_improved_create_more_constraints_ILP(alloc, alloc_X, alloc_Y, allocation_variables))
            alloc_X = second_improved_find_envy_free_allocation(alloc, allocation_variables, constraints_ilp)
...
@jit(nopython=True)
# nopython=True is used to compile the function to machine code, which can improve performance.
# However, it is not always possible to use it, as it does not support all Python features.
def second_improved_find_envy_free_allocation(alloc: AllocationBuilder, allocation_variables, constraints_ilp):
    """
    ...
    >>> alloc_X = second_improved_find_envy_free_allocation(alloc, allocation_vars, [])
    >>> print(alloc_X)
    [[2 0 0]
     [0 0 2]
     [0 2 0]]
...
# nopython=True is used to compile the function to machine code, which can improve performance.
# However, it is not always possible to use it, as it does not support all Python features.
def second_improved_find_pareto_dominating_allocation(alloc: AllocationBuilder, alloc_matrix):
    """
    ...
    >>> pareto_optimal_allocation = second_improved_find_pareto_dominating_allocation(alloc, alloc_X)
...
# nopython=True is used to compile the function to machine code, which can improve performance.
# However, it is not always possible to use it, as it does not support all Python features.
def second_improved_create_more_constraints_ILP(alloc: AllocationBuilder, alloc_X: np.ndarray, alloc_Y: np.ndarray, allocation_variables: cp.Variable):
    """
...
# nopython=True is used to compile the function to machine code, which can improve performance.
# However, it is not always possible to use it, as it does not support all Python features.
def second_improved_calculate_values(alloc: AllocationBuilder, alloc_X: np.ndarray):
    """
...
# nopython=True is used to compile the function to machine code, which can improve performance.
# However, it is not always possible to use it, as it does not support all Python features.
def second_improved_calculate_envy_matrix(alloc: AllocationBuilder, alloc_X: np.ndarray):
    """
...
if __name__ == "__main__":
    ...
    divide(second_improved_high_multiplicity_fair_allocation, instance_4_6())
