...
def create_more_constraints_ILP(alloc: AllocationBuilder, alloc_X, alloc_Y, allocation):
    ...
    for agent in agents:
        for item in items:
            # Constraint 1
            constraints.append(allocation[i][j] + (alloc_Y[i][j] - alloc_X[i][j]) <= -1 + (2 * items_capacities[j]) * (
                        1 - Z[agent, item]))
            # Constraint 2
            constraints.append(
                allocation[i][j] + (alloc_Y[i][j] - alloc_X[i][j]) >= Z_bar[agent, item] * (items_capacities[j] + 1))
            j += 1
        i += 1
        j = 0
